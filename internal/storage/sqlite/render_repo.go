...
// FindByIdempotencyKey resolves a stored submission by its idempotency key. The
// project scope is optional so a caller that has not resolved the project yet can
// still answer a replay from the key alone.
func (r *renderRepo) FindByIdempotencyKey(ctx context.Context, projectID, key string) (*domain.RenderJob, error) {
	...
	query := "SELECT " + renderColumns + " FROM render_jobs WHERE idempotency_key = ?"
	args := []any{trimmed}
	if scope := strings.TrimSpace(projectID); scope != "" {
		query += " AND project_id = ?"
		args = append(args, scope)
	}
	query += " ORDER BY queued_at ASC"
	row := r.store.conn(ctx).QueryRowContext(ctx, query, args...)
	return scanRender(row)
}
