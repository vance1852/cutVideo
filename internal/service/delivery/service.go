...
func (s *Service) dispatchOne(ctx context.Context, actor domain.Principal, job *domain.RenderJob, target *domain.DeliveryTarget, record *domain.DeliveryRecord) domain.DeliveryOutcome {
	...
	if apierr.Interrupted(sendErr) {
		// The transfer was cut short instead of refused, so the destination keeps
		// the attempt it already has and the operator can retry later.
		logging.FromContext(ctx, s.logger).Warn("delivery attempt interrupted",
