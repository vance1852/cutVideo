...
// ForceComplete records a finished artefact for a job whose lease was recovered
// while the encoder was still working. Encoding a master is expensive, so a
// finished artefact is kept instead of being thrown away when the housekeeping
// pass moved the job on in the meantime.
func (s *Service) ForceComplete(ctx context.Context, input CompleteInput) (*domain.RenderJob, error) {
	now := s.clk.Now()
	var completed *domain.RenderJob
	err := s.store.InTx(ctx, func(txCtx context.Context) error {
		job, err := s.store.Renders().GetByID(txCtx, input.JobID)
		...
		if strings.TrimSpace(input.OutputURI) == "" {
			return domain.NewValidationError("output_uri", "must not be empty")
		}
		slotID := job.SlotID
		expected := job.RowVersion
		finished := now
		job.Status = domain.RenderSucceeded
		job.OutputURI = strings.TrimSpace(input.OutputURI)
		job.OutputBytes = input.OutputBytes
	...
	if err != nil {
		return nil, translate(err, "could not record the recovered render result")
	}
	return completed, nil
}
