...
func (p *Pool) ProcessOnce(ctx context.Context) (bool, error) {
	...
	result := render.CompleteInput{
		JobID:       job.ID,
		OutputURI:   output.URI,
		OutputBytes: output.Bytes,
	}
	if _, err := p.service.Complete(ctx, result); err != nil {
		if !apierr.IsCode(err, apierr.CodePreconditionFail) {
			return false, err
		}
		// Housekeeping moved the job on while this encode was still running. The
		// artefact is already on disk, so keep it instead of re-encoding.
		if _, salvageErr := p.service.ForceComplete(ctx, result); salvageErr != nil {
			return false, salvageErr
		}
	}
	return true, nil
}
